...
describe('when there is initially some blogs saved', () => {
  beforeEach( async () => { // reset user and blog collections
  ...
  describe('addition of a new blog', () => {
    beforeEach(async () => { // get authorization token for first user
      const userForToken = {
        username: 'user1',
        id: _idOfFirstUser
      }
    ...
    test('succeeds with valid data', async () => {
      ...
      assert.strictEqual(user, _idOfFirstUser.toString())
  ...
  describe('deletion of a blog', () => {
    ...
    test('fails if no token is provided', async () => {
      const blogsAtStart = await helper.blogsInDb()
      const blogToDelete = blogsAtStart.find(b => b.user === _idOfFirstUser.toString())
    ...
    test('fails if token and blog creator do not match', async () => {
      ...
      await api
        .delete(`/api/blogs/${blogToDelete.id}`)
        .set('Authorization', `Bearer ${tokenNotFromBlogCreator}`)
        .expect(401)
    })
